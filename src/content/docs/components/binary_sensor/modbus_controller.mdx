...
- **response_size** (*Optional*, int): Size of the response for the register in bytes. Defaults to `2`. Only needed when the device answers with a non-standard length, including a `custom_command` whose reply is longer than one register. Has no effect on the `coil` and `discrete_input` register types, which are always read as a single bit.
...
- **lambda** (*Optional*, [lambda](/automations/templates#config-lambda)):
  ...
  - **item** (const pointer to a ModbusBinarySensor object): The binary sensor object itself.
