...
- **format** (**Optional**): The format that the image is encoded with.

  - `AUTO` : The image will be decoded according to the MIME type specified by the server. If the server does not send the content-type header, or the returned MIME-type is not one of the supported types, the decoding will fail (even if the format would be supported otherwise). If you get an error while using `AUTO`, set the image format explicitly.
...
> [!NOTE]
> using `AUTO` format (or leaving the format empty) will add the libraries and functionality required to decode all supported formats; which might increase the ROM size. If you know beforehand which format the downloaded images will have, it is better to specify the format explicitly.

- **resize** (*Optional*, string): If set, this will resize the image to fit inside the given dimensions `WIDTHxHEIGHT`
  and preserve the aspect ratio.

- **placeholder** (*Optional*, [ID](/guides/configuration-types#id)): ID of an [Image](/components/image/) to display while the downloaded image is not yet ready.
...
- [Animation Component](/components/animation/)
