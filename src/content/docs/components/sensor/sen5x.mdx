...
import sen54WebImg from './images/sen54-web.png';
...
<Image src={sen54WebImg} layout="constrained" alt="" />
...
- **model** (*Optional*): Force the sensor model instead of relying on the product name
  reported by the device. One of `sen50`, `sen54` or `sen55`. By default the model is
  detected automatically. If the configured model does not match what the device reports,
  a warning is logged and the configured model is used; readings for capabilities the
  physical device does not have may be invalid.
...
<Image src={jst6pinImg} layout="constrained" alt="" />
...
- [Sen6x Series Environmental sensor](/components/sensor/sen6x/)
...
- <APIRef text="sen5x.h" path="sen5x/sen5x.h" />
