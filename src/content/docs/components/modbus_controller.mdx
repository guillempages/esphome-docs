...
- **address** (*Optional*, int): The Modbus device address (a hex byte, `0x00`-`0xFF`). Defaults to `0x01`.
...
Check out the various Modbus components available at the bottom of the document in the [See Also](#see-also) section. They can be directly defined *(inline)* under the `modbus_controller` hub or as standalone components. Technically there is no difference between the *inline* and the standard definitions approach.
...
This automation will be triggered when a command has been transmitted by the `modbus` hub. In [Lambdas](/automations/templates#config-lambda)
