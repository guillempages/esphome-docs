...
- **response_size** (*Optional*, int): Number of bytes of the response. Defaults to `2` (one register).
...
- **lambda** (*Optional*, [lambda](/automations/templates#config-lambda)):
  ...
  - **item** (const pointer to a SensorItem derived object): The text sensor object itself.
