...
- **format** (**Required**): The format that the image is encoded with.
  ...
  > [!NOTE]
  > Using `AUTO` format will add the libraries and functionality required to decode all supported formats; which will increase the binary size. If you know beforehand which format the downloaded images will have, it is better to specify the format explicitly.
