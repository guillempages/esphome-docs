...
- **ignore_efuse_custom_mac** (*Optional*, boolean): When a custom MAC address is burned into the eFuses, ESPHome
  uses it as the device's base MAC address in place of the factory-assigned one. Since ESPHome 2026.9.0, all network
  interfaces (Wi-Fi, Ethernet, Bluetooth and 802.15.4) derive their addresses from it; in earlier versions, only
  Wi-Fi did. Can be set to `true` for devices on which the burned-in custom MAC address is not usable, restoring the
  factory-assigned MAC address. Defaults to `false`.
