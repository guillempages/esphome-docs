...
- **active** (*Optional*, boolean): Enables proxying active GATT connections to BLE devices.
  ...
  or your platform's BLE tracker).
  Defaults to `true` on ESP32. On advertisement-only platforms it defaults to `false` and
  enabling it fails validation — see [Platform Support](#platform-support).
...
- **connection_slots** (*Optional*, int): ESP32 only. The maximum number of BLE connection slots to use.
...
On ESP32 the Bluetooth proxy depends on [ESP32 BLE Tracker](/components/esp32_ble_tracker) so make sure to
add that to your configuration.
...
- **ESP32** — the full proxy: advertisements plus **active connections**, so Home Assistant can read
  and write GATT characteristics through the device (`active: true`, `connection_slots`).
- **LN882x and RP2 (Raspberry Pi Pico W family)** — **advertisement-only**. These controllers do
  not expose the GATT client the proxy needs for connections, so Home Assistant uses the device
  for advertisements only and will not route connections through it. This is independent of the
...
must be part of the configuration — [LN882H BLE Tracker](/components/ln882h_ble_tracker/) or
