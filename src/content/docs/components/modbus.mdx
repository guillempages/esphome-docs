...
Function code 23 (`0x17`) is implemented for the `server` role only; see
...
Ad-hoc requests from automations and lambdas are provided by the [Modbus Client](/components/modbus_client/)
actions; `modbus_client.send` without reply handlers is the fire-and-forget way to put a frame on the bus.
